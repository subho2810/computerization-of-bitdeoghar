...
import java.sql.*;
import java.awt.*;
class loginboys extends JFrame{
   ...
   Statement st;
	public loginboys() {
	    ...
	    JLabel c=new JLabel(new ImageIcon("maincodes/images/loginboys.png"));
	    h.add(c);
		...
		name=new JTextField("");
		Name.setBounds(840,250,120,50);
		Name.setFont(font2);
		name.setBounds(900,250,300,40);
		name.setFont(font);
		Password=new JLabel("PASSWORD");
        pswd=new JPasswordField("");
		Password.setFont(font2);
		pswd.setFont(font);
		Password.setBounds(840,350,100,40);
		pswd.setBounds(950,350,200,40);
		name.setBorder(br);
        pswd.setBorder(br);
        login =new JButton("LOGIN");
		login.setFont(font2);
		login.setBounds(840,470,150,50);
        signup =new JButton("Sign Up/Register");
		signup.setFont(font2);
		signup.setBounds(1000,470,200,50);
		goback=new JButton("Go BACK");
        goback.setBounds(930,570,120,70);
        ...
        signup.addActionListener(new ActionListener(){
            ...
            @Override
            public void actionPerformed(ActionEvent e) {
                boysHostelregister rg=new boysHostelregister();
                setVisible(false);
                rg.setVisible(true);
        ...
        login.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                try{
                      ...
                      String query="Select Boys_name,password,rollno from boys where Boys_name='"+NAME+"' and password='"+PASSWORD+"'";
                      ResultSet res=st.executeQuery(query);
                      if(res.next()==true){
                        JOptionPane.showMessageDialog(null," ✌ Press OK To LOGIN ✌","Message",JOptionPane.INFORMATION_MESSAGE);
                        BoysHosteldashboard oh=new BoysHosteldashboard(NAME);
                        setVisible(false);
                        oh.setVisible(true);
                      }
                      else{
